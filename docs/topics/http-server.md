...
flowchart TB
    ...
    subgraph INNER[HTTP 接入层内部]
        HS[HttpServer 编排器
        多平面创建、端口策略]
        HH[HttpHandler 路由注册器
        Business/Management/Metrics 三平面]
        Sec[TLS 安全层
        HttpSsl / HttpSslSecret]
        Obs[可观测性层
        HttpMetrics / PrometheusMetrics]
        DMI[DMI 分布式层
        DmiRole / FlexP%Processor]
    end
...
flowchart LR
    ...
    HL --> Pre[pre_routing_handler
        生成 UUID + startTime]
    Pre --> HH[HttpHandler::HandlePostGenerate]
    HH --> DI[DispatchInfer
        buildFn → InferInterface]
    DI --> SRI[SingleReqInferInterface
        推理引擎调用]
    SRI --> HDR[HandleDResult
        res.set_content json]
    HDR --> Post[post_routing_handler
        AddTracerData + RemoveMonitorRequest]
