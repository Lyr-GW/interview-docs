...
!!! abstract "30 秒速览"
    - LLM/VLM 在推理时并未遵循信息瓶颈原则——残差流和最终层 Logits 几乎原封不动地保留输入中的所有隐私信息（物体属性、背景细节、噪声类型）
    - 通过轻量级分类器（探针）可从这些表示中极高准确率地恢复出提示词中未提及的敏感属性，且前 30-80 个 Logits 是信息泄露的重灾区，其泄露能力与深层残差流相当，彻底打破了“灰盒 API 安全”的假设
    - !!! abstract "30 秒速览"
    - (核心要点从上文提取)
