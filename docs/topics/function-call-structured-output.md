...
!!! abstract "30 秒速览"
    - Function Call（Tool Use）是 LLM 生成工具调用指令的能力，MindIE 通过 ToolCallsProcessor 多模型族适配器 + JSON Completor 递归下降补全 + Hard Cut-off 反幻觉
    - 结构化输出（Guided Decoding）用 xgrammar 字节级下推自动机 + 预计算掩码缓存 硬性保证格式合法
    - 两者本为一体：Tool Call 是结构化输出的特化子集，业界正通过 Structural Tag（动态切换“无约束/有约束”）将约束生成与协议解析收敛，MindIE 在此路线上有明确的演进空间
    - !!! abstract "30 秒速览"
    - (核心要点从上文提取)
